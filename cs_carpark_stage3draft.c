...
struct car_space {
    ...
    int parking_rate;
    int licence_plate;
    int occupied_since; // this is the total time calculated 
...
bool carwash_space_available ( struct car_space carpark[CARPARK_ROWS][CARPARK_COLUMNS], int row, int col);
bool surrounding_is_carwash (struct car_space carpark[CARPARK_ROWS][CARPARK_COLUMNS], int row, int col);
bool if_all_carpark_is_carwash (struct car_space carpark[CARPARK_ROWS][CARPARK_COLUMNS], int row, int col);
...
int calculate_distance(int row, int col, struct car_space carpark[CARPARK_ROWS][CARPARK_COLUMNS]);
bool cable_length (struct car_space carpark[CARPARK_ROWS][CARPARK_COLUMNS], int car_row, int car_col);
void charge_car(struct car_space carpark[CARPARK_ROWS][CARPARK_COLUMNS]);
...
void carwash_bays (struct car_space carpark[CARPARK_ROWS][CARPARK_COLUMNS]){
    ...
    // bool unvalid_spacing = false;

    // bool checking_carwash_around = false;

    //NEED TO DECLARE
    // int EMPTY_OR_CARWASH = (EMPTY == -1 || CARWASH_BAY == 2);

    scanf(" %d %d %d", &row, &col, &build_time);

    if (row >= 0 && row < CARPARK_ROWS && col >= 0 && col < CARPARK_COLUMNS) {
            ...
            ((col == CARPARK_COLUMNS - 1 || carpark[row][col + 1].licence_plate == EMPTY) ||
            ...
            if (checking_spaces_around) {
                ...
                //assign buil_time to something in the future but as for now i'll keep this here:
                // carpark[row][col].occupied_since = build_time;
                printf("Carwash bay has been built on space (%d, %d) at time %d.", row, col, build_time);
...
bool carwash_space_available ( struct car_space carpark[CARPARK_ROWS][CARPARK_COLUMNS], int row, int col) {
    ...
    for (int inner_row = 0; inner_row < CARPARK_ROWS; inner_row++) {
        for (int inner_col = 0; inner_col < CARPARK_COLUMNS; inner_col++) {
            if (carpark[inner_row][inner_col].type == CARWASH_BAY && !carpark[inner_row][inner_col].carwash_spaces) {
...
void moving_cars (struct car_space carpark[CARPARK_ROWS][CARPARK_COLUMNS]){
    ...
    if (carpark[new_row][new_col].type == CARWASH_BAY) {
        if (!surrounding_is_carwash(carpark, new_row, new_col)) {
            ...
            return;
        } else if (if_all_carpark_is_carwash(carpark, new_row, new_col)) {
            printf("Car %4d is now using the carwash bay located at (%d, %d).", licence, new_row, new_col);
            remove_car(carpark, row, col);
            return;
        } else if (!if_all_carpark_is_carwash(carpark, new_row, new_col)) {
    ...
    if(surrounding_is_carwash(carpark, new_row, new_col) && carpark[new_row][new_col].type != CARWASH_BAY) {
        if (!carwash_space_available(carpark, new_row, new_col)) {
            ...
            // } else if (surrounding_is_carwash(carpark, new_row, new_col) && carpark[new_row][new_col].type != CARWASH_BAY) {
            // if (!carwash_space_available(carpark, new_row, new_col)) {
            //     printf("Cannot park next to a carwash bay. There are still other spaces available in the carpark.");
            //     return;
            // } else if (carwash_space_available(carpark, new_row, new_col)) {
    ...
    if (carpark[new_row][new_col].licence_plate == EMPTY &&
        ...
        if ((time_of_move <= carpark[new_row][new_col].occupied_since)||
            (time_of_move <= (carpark[new_row][new_col].normal_occupied_since + carpark[new_row][new_col].premium_occupied_since))) {
        ...
        for (int row = 0; row < CARPARK_ROWS; row++) {
            for (int col = 0; col < CARPARK_COLUMNS; col++) {
                ...
                } else if (carpark[row][col].licence_plate != licence) {
...
void carwash_function(struct car_space carpark[CARPARK_ROWS][CARPARK_COLUMNS],
              int licence_plate_no, int row, int col, int time_of_entry) {
    if (!if_all_carpark_is_carwash(carpark, row, col) && !carwash_space_available(carpark, row, col)) {
        carpark[row][col].licence_plate = licence_plate_no;
    ...
    if (row < CARPARK_ROWS && col < CARPARK_COLUMNS) {
        ...
        if (carpark[row][col].licence_plate == EMPTY &&
         ...
         (!surrounding_is_carwash (carpark, row, col))) {
            ...
            printf("Cannot park on a space occupied by an electric charging");
            printf(" station.\n");
            ...
            if (!carwash_space_available(carpark, row, col)) {
        ...
        //checking if there are still carspace available
...
void charge_car(struct car_space carpark[CARPARK_ROWS][CARPARK_COLUMNS]) {
    ...
    for (int row = 0; row < CARPARK_ROWS; row++) {
        for (int col = 0; col < CARPARK_COLUMNS; col++) {
            if (carpark[row][col].type == ELECTRIC_CHARGING_STATION) {
                ...
                if (min_distance > distance_of_two(row, col, car_row, car_col)){
    ...
    carpark[charge_row][charge_col].deactivated = 0;

    if ((carpark[charge_row][charge_col].previous_from != -1) && (charge_from > carpark[charge_row][charge_col].previous_from)) {
        carpark[charge_row][charge_col].deactivated = -1;
    }
    ...
    if (carpark[car_row][car_col].licence_plate == licence_plate_no) {
        ...
        printf("Car %4d is being charged by the charging station located at (%d, %d).", 
        licence_plate_no, charge_row, charge_col);
        
        // carpark[charge_row][charge_col].previous_from = charge_time;
...
double distance_of_two (int row_1, int col_1, int row_2, int col_2) {
    ...
    if (row_1 >= 0 && col_1 >= 0 && row_2 >= 0 && col_2){
        int row = 0;
        int col = 0;
...
void build_electric_station(struct car_space carpark[CARPARK_ROWS][CARPARK_COLUMNS]){
    ...
    if (row < CARPARK_ROWS && col < CARPARK_COLUMNS) {
        if (carpark[row][col].licence_plate == EMPTY || 
            ...
            carpark[row][col].previous_from = -1;
...
void find_car(struct car_space carpark[CARPARK_ROWS][CARPARK_COLUMNS], 
    ...
    // Flag to indicate if the car was found
    int found_car = 0; 
    ...
    for (int row = 0; row < CARPARK_ROWS; row++) {
        for (int col = 0; col < CARPARK_COLUMNS; col++) {
            ...
            if ((carpark[row][col].licence_plate == licence_plate_no)
                ...
                //below
                calculate_price_of_one (carpark, row, col,
                 licence_plate_no, time_of_exit,total_time, total_price, discount);
